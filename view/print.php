...
<body>
    <div class="container">
        ...
        $kode = $_POST['kode'];
        if ($_POST['bulan'] == 'all') {
            $query = "SELECT * FROM pembayaran_spp INNER JOIN tb_petugas USING(id_petugas) INNER JOIN tb_siswa USING(nis) WHERE     
            nama_kelas like '%$kode%' or
            ...
            $query = "SELECT * FROM pembayaran_spp INNER JOIN tb_petugas USING(id_petugas) INNER JOIN tb_siswa USING(nis) WHERE bulan='$list_bulan' AND nama_kelas IS NOT NULL AND nama_kelas like '%$kode%'";
            // $hasil = mysqli_query($koneksi, $query);
        }
        ?>
